...
release-linux-x86_64:
	...
	CARGO_TARGET_X86_64_UNKNOWN_LINUX_GNU_LINKER=x86_64-linux-gnu-gcc \
		cargo build --release --target x86_64-unknown-linux-gnu -p rsync -p rc -p apiserver
	mkdir -p bin
...
release-linux-aarch64:
	...
	CARGO_TARGET_AARCH64_UNKNOWN_LINUX_GNU_LINKER=aarch64-linux-gnu-gcc \
		cargo build --release --target aarch64-unknown-linux-gnu -p rsync -p rc -p apiserver
	mkdir -p bin
...
release-darwin-x86_64:
	...
	cargo build --release --target x86_64-apple-darwin -p rsync -p rc -p apiserver
	mkdir -p bin
...
release-darwin-aarch64:
	...
	cargo build --release --target aarch64-apple-darwin -p rsync -p rc -p apiserver
	mkdir -p bin
