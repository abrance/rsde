...
export default function PromptPage() {
    ...
    const [viewedTemplate, setViewedTemplate] = useState<PromptTemplate | null>(null)
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            if (variables.trim()) requestBody.variables = variables.split(',').map(v => v.trim()).filter(v => v)
    ...
    const handleUpdate = async () => {
        ...
        try {
            const requestBody = {
                ...
                variables: editForm.variables.split(',').map(v => v.trim()).filter(v => v),
                tags: editForm.tags.split(',').map(t => t.trim()).filter(t => t),
                is_active: editForm.is_active,
            }

            const response = await fetch(`/api/prompt/template/${viewedTemplate.id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(requestBody),
            })
        ...
        <div className="tool-page">
            <div className="page-header">
                ...
                <p className="page-description">
                    Manage prompt templates for AI applications with versioning and categorization
                </p>
            </div>
                    ...
                    <div className="overview">
                        <div className="card">
                            ...
                            <ul className="feature-list">
                                ...
                                <li>📂 Categorize prompts (Chat, Completion, Assistant, Agent, Custom)</li>
                        ...
                        <div className="card">
                            ...
                            <div className="format-grid">
                                {CATEGORIES.map(cat => (
                                    <span key={cat.value} className="format-badge">{cat.label}</span>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="card">
                            ...
                            <ol style={{ lineHeight: '1.8' }}>
                                ...
                                <li>Define variables using placeholders like {'{{variable_name}}'}</li>
                        ...
                        <div className="card">
                            ...
                            <div className="form-group" style={{ marginBottom: '20px' }}>
                                <div style={{ display: 'flex', gap: '10px' }}>
                                    ...
                                    <button className="btn" onClick={() => { setSearchName(''); fetchTemplates() }} style={{ backgroundColor: '#6c757d' }}>
                                        Clear
                                    </button>
                                </div>
                            </div>
                                ...
                                <>
                                    <div className="template-list">
                                        {templates.map((template) => (
                                            <div key={template.id} className="template-item card" style={{ marginBottom: '15px', padding: '15px' }}>
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                                                    <div style={{ flex: 1 }}>
                                                        <h3 style={{ margin: '0 0 10px 0', display: 'flex', alignItems: 'center', gap: '10px' }}>
                                                            {template.name}
                                                            <span className="format-badge" style={{ fontSize: '0.8em' }}>
                                                                {getCategoryLabel(template.category)}
                                                            </span>
                                                            {!template.is_active && (
                                                                <span style={{ fontSize: '0.8em', color: '#dc3545' }}>(Inactive)</span>
                                                            )}
                                                        </h3>
                                                        ...
                                                        {template.tags.length > 0 && (
                                                            <div style={{ marginBottom: '10px' }}>
                                                                {template.tags.map((tag, i) => (
                                                                    <span key={i} className="format-badge" style={{ marginRight: '5px', backgroundColor: '#6c757d' }}>
                                                                        {tag}
                                                                    </span>
                                                                ))}
                                                            </div>
                            ...
                            {viewedTemplate && !isEditing && (
                                <div className="result-box">
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '15px' }}>
                                        ...
                                        <div style={{ display: 'flex', gap: '10px' }}>
                                            <button className="btn" onClick={startEdit} style={{ padding: '5px 15px' }}>
                                                ✏️ Edit
                                            </button>
                                            <button
                                                className="btn"
                                    ...
                                    <div style={{ fontSize: '0.9em', color: '#666', marginBottom: '15px' }}>
                                        ...
                                        <p>📂 Category: {getCategoryLabel(viewedTemplate.category)}</p>
                                        {viewedTemplate.description && <p>📝 Description: {viewedTemplate.description}</p>}
                                        <p>📋 Version: {viewedTemplate.version}</p>
                                        <p>✅ Status: {viewedTemplate.is_active ? 'Active' : 'Inactive'}</p>
                                        {viewedTemplate.created_by && <p>👤 Created by: {viewedTemplate.created_by}</p>}
                                        <p>🕒 Created: {formatDateTime(viewedTemplate.created_at)}</p>
                                        <p>🔄 Updated: {formatDateTime(viewedTemplate.updated_at)}</p>
                                        {viewedTemplate.variables.length > 0 && (
                                            <div style={{ marginTop: '10px' }}>
                                                <span>📌 Variables: </span>
                                                {viewedTemplate.variables.map((v, i) => (
                                                    <span key={i} className="format-badge" style={{ marginRight: '5px', backgroundColor: '#17a2b8' }}>
                                                        {v}
                                                    </span>
                                                ))}
                                            </div>
                            ...
                            {viewedTemplate && isEditing && editForm && (
                                <div className="result-box">
                                    ...
                                    <div style={{ display: 'flex', gap: '10px' }}>
                                        ...
                                        <button className="btn" onClick={cancelEdit} style={{ backgroundColor: '#6c757d' }}>
                                            Cancel
                                        </button>
                                    </div>
                                </div>
