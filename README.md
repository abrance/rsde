...
- **datalink-engine** - 数据链路元数据管理 crate，负责 datasource / etl_pipeline / result_table 的统一登记与查询
...
# 2. 启动服务（如需启用 datalink-engine，请确保配置中包含 [datalink_engine]）
...
- OCR 图片文字识别
- 统一登记和查询数据链路元数据，并通过 result table 查找数据落点
...
├── datalink-engine/    # 数据链路元数据管理 crate
├── rsync/             # 文件同步工具
├── rc/                # 远程配置管理
├── pic_recog/         # OCR 识别模块
├── anybox/            # 文本分享服务（类似 Pastebin）
├── webserver/
...
- ✅ **OCR 图片识别**
- ✅ **图床服务**
- ✅ **DataLink 元数据管理与 `/api/datalink/v1` API**
- 🚧 Prometheus Metrics（开发中）
- 🚧 MCP 协议支持（开发中）

## DataLink Engine

当前分支已接入 `datalink-engine`：

- `apiserver` 在配置存在时挂载 `/api/datalink/v1`
- `common/config` 支持 `[datalink_engine]` 配置块
- 当前可用后端：`backend = "memory"`
- `backend = "mysql"` 的配置面已经预留，但后端实现仍返回未支持
